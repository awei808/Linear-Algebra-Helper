...
(function () {
    ...
    //*************** CLASSES ***************//
    /**
     * 多项式类构造函数 - 将符号转换为多项式系数数组
     * 生成格式为 [[系数1, 幂次1], [系数2, 幂次2], ...] 的多项式表示
     * 仅支持单变量多项式
     * @param {Symbol|Number} symbol - 输入符号或数字
     * @param {String} variable - 多项式的变量名
     * @param {int} order - 多项式阶数
     */
     ...
     * 多项式拟合函数 - 根据给定条件生成多项式
     * 通过模10运算找到匹配因子来构建多项式
     * @param {Number} c1 - x^p项的系数
     * @param {Number} c2 - 常数项系数
     * @param {Number} n - 目标值
     * @param {Number} base - 基数
     * @param {Number} p - 多项式阶数
     * @param {String} variable - 变量名
     * @returns {Polynomial|null} 拟合的多项式或null（如果无法拟合）
     */
    /*
    ...
    Polynomial.prototype = {
         ...
         * 将符号转换为多项式表示
         * 解析数学符号并提取多项式系数
         * @param {Symbol} symbol - 要解析的数学符号
         * @param {Array} c - 系数收集数组
         * @returns {Polynomial} 当前多项式对象
         */
         ...
         * 多项式加法运算
         * 将两个多项式相加，对应系数相加
         * @param {Polynomial} poly - 要相加的多项式
         * @returns {Polynomial} 相加后的多项式
         */
        add: function (poly) {
         ...
         * 多项式减法运算
         * 从当前多项式减去另一个多项式
         * @param {Polynomial} poly - 要减去的多项式
         * @returns {Polynomial} 相减后的多项式
         */
         ...
         * 多项式除法运算
         * 执行多项式长除法，返回商和余数
         * @param {Polynomial} poly - 除数多项式
         * @returns {Array} 包含商和余数的数组 [商, 余数]
         */
         ...
         * 多项式乘法运算
         * 将两个多项式相乘，使用卷积方法计算系数
         * @param {Polynomial} poly - 要相乘的多项式
         * @returns {Polynomial} 相乘后的多项式
         */
         ...
         * 检查多项式是否为零多项式
         * 验证所有系数是否都为零
         * @returns {Boolean} 如果所有系数为零则返回true
         */
        isZero: function () {
            var l = this.coeffs.length;
            for (var i = 0; i < l; i++) {
         ...
         * 多项式求值运算
         * 将数值n代入多项式p(n)进行计算
         * @param {Number} n - 要代入的数值
         * @returns {Frac} 多项式在n处的值
         */
        sub: function (n) {
         ...
         * 计算多项式的最大公因子（GCF）
         * 返回多项式系数的最大公约数
         * @param {bool} toPolynomial - 是否返回多项式形式
         * @returns {Frac|Polynomial} 最大公因子（分数或多项式）
         */
         ...
         * 计算二次多项式的根
         * 使用求根公式解二次方程
         * @param {bool} incl_img - 是否包含虚数根
         * @returns {Array} 多项式的根数组
         */
        ...
        /**
         * 无平方分解 - 将多项式分解为无平方因子的形式
         * 通过连续求导和GCD运算去除平方因子
         * @returns {Array} 包含无平方多项式、剩余因子和迭代次数的数组
         */
        squareFree: function () {
         ...
         * 检查多项式是否等于特定数值
         * 验证多项式是否为常数且等于给定数值
         * @param {Number} x - 要比较的数值
         * @returns {Boolean} 如果多项式等于该数值则返回true
         */
    ...
    /**
     * 符号系数提取方法
     * 如果符号属于PL或CP组，它将返回每个符号的乘数作为多项式系数。
     * CB符号通过乘法粘合在一起，因此符号乘数携带所有包含符号的系数。
     * 对于S组，它只返回自身的乘数。此函数不关心是否为多项式。
     * @param {Array} c - 系数数组
     * @param {boolean} with_order - 是否包含阶数信息
     * @return {Array} 系数数组
     */
    ...
    var __ = core.Algebra = {
        ...
        Factor: {
            ...
            /**
             * 因式分解主函数 - 将数学符号分解为不可约因子
             * @param {Symbol} symbol - 要分解的数学符号（表达式）
             * @param {Factors} factors - 因子容器对象，用于存储分解结果
             * @returns {Symbol} 分解后的符号表达式
             */
            factor: function (symbol, factors) {
                // 如果符号是常数，直接调用数学库的质因数分解
                ...
                // 如果分解结果与原符号相同，说明无法进一步分解，直接返回
                if (retval.equals(symbol)) {
                    return retval;
                }

                // 处理 CB 组（乘法组合表达式）的特殊情况
                if (retval.group === CB) {
                    var t = new Symbol(1);  // 临时存储因子乘积
                    var p = _.parse(retval.power);  // 提取幂次
                    // 存储乘数并剥离它
                    ...
                    if (retval.multiplier.lessThan(0)) {
                        t.negate();  // t 取负
                        retval.negate();  // 原符号取负（抵消）
                    }

                    // 遍历 CB 组中的每个子符号进行递归分解
                    retval.each(function (x) {
                        // 与问题 #566 相关：由于符号的分组可能未正确更新
                        // 更简单的方法是重新解析符号，让解析器为我们更新分组
                        var factored = _.parse(__.Factor._factor(x));

                        // 如果分解结果仍然是 CB 组（嵌套的乘法表达式）
