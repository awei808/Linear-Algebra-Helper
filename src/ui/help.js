...
export function switchContent(isNext) {
    if (isNext) {
        ...
        state.helpContentIndex = (state.helpContentIndex - 1 + helpContent.length) % helpContent.length;
    }
...
export function createNavigationDots() {
    ...
    for (let i = 0; i < helpContent.length; i++) {
        ...
        dot.addEventListener('mouseleave', function () {
            if (parseInt(this.dataset.index) !== state.helpContentIndex) {
...
export function updateNavigationDots() {
    ...
    dots.forEach((dot, index) => {
        if (index === state.helpContentIndex) {
