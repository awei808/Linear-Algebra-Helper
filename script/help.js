...
const helpContent = [
    ...
    '<h4>这种矩阵录入方式太麻烦了，有没有更简单的录入方式？</h4><p>有的！点击"更多"按钮，选择"导入二维数组为矩阵"，可以输入标准二维数组格式的矩阵数据，例如：[[1,2],[3,4]]。输入完成后点击录入矩阵按钮，即可完成录入。</p>',

    '<h4>这些目标行/列，系数，参数行/列，以及变换符号，是什么意思，有什么用？</h4><p>目标行/列：你要进行变换的那一行或列</p><p>系数：变换时乘的倍数</p><p>参数行/列：参与变换的另一行或列</p><p>变换符号：↔（交换目标行列和参数行列的位置）、+（向目标行列加上系数*参数行列）、−（向目标行列减去系数*参数行列）、×（对目标行列倍乘）</p><p>这些是进行初等变换的基本参数。</p>',

    '<h4>怎样进行初等变换？</h4><p>1. 先点击要变换的行或列（如r1、c2）</p><p>2. 选择变换符号（↔、+、−、×）</p><p>3. 如果需要，输入系数和参数行/列</p><p>4. 点击"执行初等变换"按钮完成变换</p>',

    '<h4>怎样选择要变换的行或列，以及符号？</h4><p>1.先点击“目标行/列”，再点击矩阵表格中对应的行或列（如r1、c2等），即可选中；参数行/列同理</p><p>2.点击中间的↔、+、−、×按钮即可选中</p>',

    '<h4>我输错了初等变换数据，还点击了"执行初等变换"按钮，怎么办？</h4><p>不用担心！点击"撤销初等变换"按钮可以撤销上一步操作。如果需要重做，点击"重做初等变换"按钮。系统支持多步撤销和重做。</p>',

    '<h4>初等变换后，矩阵元素好复杂，看不出元素之间的关系，怎么办？</h4><p>1.点击你认为复杂的矩阵元素 2.点击"更多"按钮，选择"多项式展开"可以展开复杂的数学表达式。选择"一元多项式因式分解"可以因式分解。</p><p>若对处理结果不满意，可以点击"替换矩阵元素"按钮对单个矩阵元素进行替换。</p>',
